...
Kluczowe decyzje projektowe:
    ...
    - Pula procesów tworzona jest RAZ (create_parallel_context) i
      wykorzystywana wielokrotnie dla wielu grafów – koszt startu
      workerów jest jednorazowy.
    - Pamięć współdzielona jest pre-alokowana do maksymalnych rozmiarów
      i re-używana między grafami (bez re-alokacji).
...
def create_parallel_context(num_workers=None, max_nodes=MAX_NODES, max_edges=MAX_EDGES):
    """
    Tworzy kontekst równoległy: pulę procesów i współdzieloną pamięć.

    Wywoływane JEDNORAZOWO przed przetworzeniem wszystkich grafów.
    Pula procesów i pamięć współdzielona są re-używane między grafami.

    Parametry:
        num_workers : liczba procesów roboczych (domyślnie: liczba rdzeni CPU)
        max_nodes   : maksymalna liczba wierzchołków w grafie
        max_edges   : maksymalna liczba krawędzi w grafie

    Zwraca:
        context : dict z pulą, pamięcią współdzieloną i meta-danymi
    """
    if num_workers is None:
        num_workers = mp.cpu_count()

    offsets_shm = RawArray(ctypes.c_int64, max_nodes + 1)
    edges_shm = RawArray(ctypes.c_int64, max_edges)
    visited_shm = RawArray(ctypes.c_int32, max_nodes)
    pool = mp.Pool(
    ...
    return {
        'pool': pool,
        'offsets_shm': offsets_shm,
        'edges_shm': edges_shm,
        'visited_shm': visited_shm,
...
def destroy_parallel_context(context):
    """
    Zamyka pulę procesów. Wywoływane po przetworzeniu WSZYSTKICH grafów.
    """
    pool = context['pool']
    pool.close()
    pool.join()
...
def parallel_bfs(graph, context):
    ...
    """
    pool = context['pool']
    offsets_shm = context['offsets_shm']
    edges_shm = context['edges_shm']
    visited_shm = context['visited_shm']
    ...
    offsets_shm[:n + 1] = (ctypes.c_int64 * (n + 1))(*offsets)

    if m > 0:
        edges_shm[:m] = (ctypes.c_int64 * m)(*edges)
    ...
    PARALLEL_THRESHOLD = max(512, num_workers * 32)
